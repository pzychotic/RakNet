...
public:
	...
	/// \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
	bool PushConnection(Connection_RM3 *newConnection, WorldId worldId=0);
	...
	/// \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
	Connection_RM3 * PopConnection(RakNetGUID guid, WorldId worldId=0);
	...
	/// \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
	void Reference(Replica3 *replica3, WorldId worldId=0);
	...
	/// \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
	void Dereference(Replica3 *replica3, WorldId worldId=0);
...
protected:
	...
	void DeallocReplicaNoBroadcastDestruction(Connection_RM3 *connection, Replica3 *replica3);
	Connection_RM3 * PopConnection(unsigned int index, WorldId worldId);
	Replica3* GetReplicaByNetworkID(NetworkID networkId, WorldId worldId);
	unsigned int ReferenceInternal(Replica3 *replica3, WorldId worldId);
...
{
	BitStream bitStream[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS];
...
struct SerializeParameters
{
	...
	BitStream outputBitstream[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS];

	/// Last bitstream we sent for this replica to this system.
	/// Read, but DO NOT MODIFY
	BitStream* lastSentBitstream[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS];
...
{
	BitStream serializationBitstream[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS];
...
public:
	...
	virtual Replica3 *AllocReplica(BitStream *allocationIdBitstream, ReplicaManager3 *replicaManager3)=0;
	...
	virtual void SerializeOnDownloadStarted(BitStream *bitStream) {(void) bitStream;}

	/// Receives whatever was written in SerializeOnDownloadStarted()
	/// \param[in] bitStream Written in SerializeOnDownloadStarted()
	virtual void DeserializeOnDownloadStarted(BitStream *bitStream) {(void) bitStream;}
	...
	virtual void SerializeOnDownloadComplete(BitStream *bitStream) {(void) bitStream;}

	/// Receives whatever was written in DeserializeOnDownloadComplete()
	/// \param[in] bitStream Written in SerializeOnDownloadComplete()
	virtual void DeserializeOnDownloadComplete(BitStream *bitStream) {(void) bitStream;}
	...
	/// \param[in] worldId Which world, see ReplicaManager3::AddWorld()
	/// \param[in] curTime The current time
	virtual SendSerializeIfChangedResult SendSerializeIfChanged(LastSerializationResult *lsr, SerializeParameters *sp, RakPeerInterface *rakPeer, unsigned char worldId, ReplicaManager3 *replicaManager, RakNet::Time curTime);
	...
	virtual void SendConstruction(DataStructures::List<Replica3*> &newObjects, DataStructures::List<Replica3*> &deletedObjects, PRO sendParameters, RakPeerInterface *rakPeer, unsigned char worldId, ReplicaManager3 *replicaManager3);

	/// \internal
	void SendValidation(RakPeerInterface *rakPeer, WorldId worldId);
...
protected:
	...
	*/
	...
	void SendSerializeHeader(Replica3 *replica, RakNet::Time timestamp, BitStream *bs, WorldId worldId);
...
public:
	...
	virtual void WriteAllocationID(Connection_RM3 *destinationConnection, BitStream *allocationIdBitstream) const=0;
	...
	virtual RM3DestructionState QueryDestruction(Connection_RM3 *destinationConnection, ReplicaManager3 *replicaManager3) {(void) destinationConnection; (void) replicaManager3; return RM3DS_DO_NOT_QUERY_DESTRUCTION;}
	...
	virtual void SerializeConstructionExisting(BitStream *constructionBitstream, Connection_RM3 *destinationConnection) {(void) constructionBitstream; (void) destinationConnection;};

	/// Same as DeserializeConstruction(), but for an object that already exists on the remote system.
	/// Used if you return RM3CS_ALREADY_EXISTS_REMOTELY from QueryConstruction
	virtual void DeserializeConstructionExisting(BitStream *constructionBitstream, Connection_RM3 *sourceConnection) {(void) constructionBitstream; (void) sourceConnection;};
	...
	virtual void OnPoppedConnection(Connection_RM3 *droppedConnection) {(void) droppedConnection;}
	...
	virtual void OnSerializeTransmission(BitStream *bitStream, Connection_RM3 *destinationConnection, BitSize_t bitsPerChannel[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS], RakNet::Time curTime) {(void) bitStream; (void) destinationConnection; (void) bitsPerChannel; (void) curTime;}
	...
	virtual void PostSerializeConstruction(BitStream *constructionBitstream, Connection_RM3 *destinationConnection) {(void) constructionBitstream; (void) destinationConnection;}
	...
	/// \param[in] sourceConnection System that sent us this network message.
	virtual void PostDeserializeConstruction(BitStream *constructionBitstream, Connection_RM3 *sourceConnection) {(void) constructionBitstream; (void) sourceConnection;}

	/// Same as PostSerializeConstruction(), but for objects that returned RM3CS_ALREADY_EXISTS_REMOTELY from QueryConstruction
	virtual void PostSerializeConstructionExisting(BitStream *constructionBitstream, Connection_RM3 *destinationConnection) {(void) constructionBitstream; (void) destinationConnection;}

	/// Same as PostDeserializeConstruction(), but for objects that returned RM3CS_ALREADY_EXISTS_REMOTELY from QueryConstruction
	virtual void PostDeserializeConstructionExisting(BitStream *constructionBitstream, Connection_RM3 *sourceConnection) {(void) constructionBitstream; (void) sourceConnection;}
	...
	/// \param[in] sourceConnection System that sent us this network message.
	virtual void PreDestruction(Connection_RM3 *sourceConnection) {(void) sourceConnection;}
	...
	/// \param[in] sourceConnection destinationConnection parameter passed to QueryConstruction()
	/// \param[in] isThisTheServer True if this system is the server, false if not.
	virtual bool QueryRemoteConstruction_ServerConstruction(Connection_RM3 *sourceConnection, bool isThisTheServer);
	...
	virtual RM3ConstructionState QueryConstruction_PeerToPeer(Connection_RM3 *destinationConnection, Replica3P2PMode p2pMode=R3P2PM_SINGLE_OWNER);
	...
	/// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
	/// \param[in] isThisTheServer True if this system is the server, false if not.
	virtual RM3QuerySerializationResult QuerySerialization_ClientSerializable(Connection_RM3 *destinationConnection, bool isThisTheServer);
	/// \brief Default call for QuerySerialization().
	...
	/// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
	/// \param[in] isThisTheServer True if this system is the server, false if not.
	virtual RM3QuerySerializationResult QuerySerialization_ServerSerializable(Connection_RM3 *destinationConnection, bool isThisTheServer);
	/// \brief Default call for QuerySerialization().
	...
	/// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
	/// \param[in] p2pMode If controlled only by this system ever, pass R3P2PM_SINGLE_OWNER. Otherwise pass R3P2PM_MULTI_OWNER_CURRENTLY_AUTHORITATIVE or R3P2PM_MULTI_OWNER_NOT_CURRENTLY_AUTHORITATIVE
	virtual RM3QuerySerializationResult QuerySerialization_PeerToPeer(Connection_RM3 *destinationConnection, Replica3P2PMode p2pMode=R3P2PM_SINGLE_OWNER);
	
	/// Default: If we are a client, and the connection is lost, delete the server's objects
	virtual RM3ActionOnPopConnection QueryActionOnPopConnection_Client(Connection_RM3 *droppedConnection) const;
	/// Default: If we are a server, and the connection is lost, delete the client's objects and broadcast the destruction
	virtual RM3ActionOnPopConnection QueryActionOnPopConnection_Server(Connection_RM3 *droppedConnection) const;
	/// Default: If we are a peer, and the connection is lost, delete the peer's objects
	virtual RM3ActionOnPopConnection QueryActionOnPopConnection_PeerToPeer(Connection_RM3 *droppedConnection) const;
...
public:
	...
	virtual void WriteAllocationID(Connection_RM3 *destinationConnection, BitStream *allocationIdBitstream) const {r3CompositeOwner->WriteAllocationID(destinationConnection, allocationIdBitstream);}
	virtual RM3ConstructionState QueryConstruction(Connection_RM3 *destinationConnection, ReplicaManager3 *replicaManager3) {return r3CompositeOwner->QueryConstruction(destinationConnection, replicaManager3);}
	virtual RM3DestructionState QueryDestruction(Connection_RM3 *destinationConnection, ReplicaManager3 *replicaManager3) {return r3CompositeOwner->QueryDestruction(destinationConnection, replicaManager3);}
	virtual bool QueryRemoteConstruction(Connection_RM3 *sourceConnection) {return r3CompositeOwner->QueryRemoteConstruction(sourceConnection);}
	...
	virtual void SerializeConstruction(BitStream *constructionBitstream, Connection_RM3 *destinationConnection) {r3CompositeOwner->SerializeConstruction(constructionBitstream, destinationConnection);}
	virtual bool DeserializeConstruction(BitStream *constructionBitstream, Connection_RM3 *sourceConnection) {return r3CompositeOwner->DeserializeConstruction(constructionBitstream, sourceConnection);}
	virtual void SerializeConstructionExisting(BitStream *constructionBitstream, Connection_RM3 *destinationConnection) {r3CompositeOwner->SerializeConstructionExisting(constructionBitstream, destinationConnection);}
	virtual void DeserializeConstructionExisting(BitStream *constructionBitstream, Connection_RM3 *sourceConnection) {r3CompositeOwner->DeserializeConstructionExisting(constructionBitstream, sourceConnection);}
	virtual void SerializeDestruction(BitStream *destructionBitstream, Connection_RM3 *destinationConnection) {r3CompositeOwner->SerializeDestruction(destructionBitstream, destinationConnection);}
	virtual bool DeserializeDestruction(BitStream *destructionBitstream, Connection_RM3 *sourceConnection) {return r3CompositeOwner->DeserializeDestruction(destructionBitstream, sourceConnection);}
	virtual RM3ActionOnPopConnection QueryActionOnPopConnection(Connection_RM3 *droppedConnection) const {return r3CompositeOwner->QueryActionOnPopConnection(droppedConnection);}
	virtual void OnPoppedConnection(Connection_RM3 *droppedConnection) {r3CompositeOwner->OnPoppedConnection(droppedConnection);}
	virtual void DeallocReplica(Connection_RM3 *sourceConnection) {r3CompositeOwner->DeallocReplica(sourceConnection);}
	virtual RM3QuerySerializationResult QuerySerialization(Connection_RM3 *destinationConnection) {return r3CompositeOwner->QuerySerialization(destinationConnection);}
	virtual void OnUserReplicaPreSerializeTick(void) {r3CompositeOwner->OnUserReplicaPreSerializeTick();}
	virtual RM3SerializationResult Serialize(SerializeParameters *serializeParameters) {return r3CompositeOwner->Serialize(serializeParameters);}
	virtual void OnSerializeTransmission(BitStream *bitStream, Connection_RM3 *destinationConnection, BitSize_t bitsPerChannel[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS], RakNet::Time curTime) {r3CompositeOwner->OnSerializeTransmission(bitStream, destinationConnection, bitsPerChannel, curTime);}
	virtual void Deserialize(DeserializeParameters *deserializeParameters) {r3CompositeOwner->Deserialize(deserializeParameters);}
	virtual void PostSerializeConstruction(BitStream *constructionBitstream, Connection_RM3 *destinationConnection) {r3CompositeOwner->PostSerializeConstruction(constructionBitstream, destinationConnection);}
	virtual void PostDeserializeConstruction(BitStream *constructionBitstream, Connection_RM3 *sourceConnection) {r3CompositeOwner->PostDeserializeConstruction(constructionBitstream, sourceConnection);}
	virtual void PostSerializeConstructionExisting(BitStream *constructionBitstream, Connection_RM3 *destinationConnection) {r3CompositeOwner->PostSerializeConstructionExisting(constructionBitstream, destinationConnection);}
	virtual void PostDeserializeConstructionExisting(BitStream *constructionBitstream, Connection_RM3 *sourceConnection) {r3CompositeOwner->PostDeserializeConstructionExisting(constructionBitstream, sourceConnection);}
	virtual void PreDestruction(Connection_RM3 *sourceConnection) {r3CompositeOwner->PreDestruction(sourceConnection);}
};
