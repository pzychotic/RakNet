	...
	if ( endThreads )
	{
		...
		if ( isMainLoopThreadActive == false )
		{
			...
			int errorCode;
					...
					errorCode = RakThread::Create(UpdateNetworkLoop, this, threadPriority);
...
{
	std::lock_guard<std::mutex> guard(packetReturnMutex);
	unsigned int size = packetReturnQueue.Size();
	return size;
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// void ProcessPortUnreachable( SystemAddress systemAddress, RakPeer *rakPeer )
// {
// 	(void) binaryAddress;
// 	(void) port;
// 	(void) rakPeer;
